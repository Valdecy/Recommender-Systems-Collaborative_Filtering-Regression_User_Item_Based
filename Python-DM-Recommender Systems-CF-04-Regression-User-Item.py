...
def user_item_based_model(Xdata, user_in_columns = True, pack_size = 25, iterations = 75):
    ...
    ibm = loss_function(Xdata, variable_list = gwo[:-1], user_in_columns = user_in_columns)
    return ibm, gwo
   
######################## Part 1 - Usage ####################################

df = pd.read_csv('Python-DM-Recommender Systems-03-CF-04c.txt', sep = '\t')
X = df.iloc[:,1:]
X = X.set_index(df.iloc[:,0]) # First column as row names

# Calling the Function
u_i_bm = user_item_based_model(X, user_in_columns = True, pack_size = 150, iterations = 100)
